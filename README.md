...
- [Theme Design Tokens & Typography](#theme-design-tokens--typography) — before and after rule comparison
...
Install `@sebastienrousseau/tailwindcss-config` as a development dependency:
...
Use the shared preset through Tailwind's `presets` option.

### tailwind.config.js

```js
import shared from "@sebastienrousseau/tailwindcss-config";
...
`@sebastienrousseau/tailwindcss-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of 21 single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/tailwindcss-config` solves this with three deliberate engineering choices:
...
When your project uses alternative CSS frameworks (such as Bootstrap or Vanilla Extract) or zero-runtime CSS modules.
