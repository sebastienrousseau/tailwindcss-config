...
%global npm_bare      tailwindcss-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-tailwindcss-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Tailwind CSS configuration providing standardized theme design tokens, color palettes, and typography

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/tailwindcss-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/tailwindcss-config is a shareable configuration preset with no runtime
...
install -pm 0644 tailwind.config.js %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
